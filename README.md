<div align="center">
  <img src="banner.png" alt="KaneFlow Banner" width="100%" />
  ...
  <p align="center">
    ...
    <em>Plain-English Specs → Browser Automation → Real-Time Telemetry → Agent Code Repair → Green Verification Run</em>
  </p>
</div>
...
* **Closed-Loop Timeline:** Step-by-step visual cards tracking every stage (`INITIAL_RUN` → `KANE_FAILED` → `DIAGNOSING` → `APPLYING_PATCH` → `VERIFIED_GREEN`).
...
* **Closed Loop**: Complete autonomous loop: Red test failure → AST diagnosis → AI code patch → Kane re-verification → Green proof.
